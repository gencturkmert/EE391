x_periodic = @(t) -1*(t>=0 & t<1) + 1*(t>=1 & t<2);

figure;
plot(t_period, x_periodic(mod(t_period,2)), 'b-', 'LineWidth', 2);
...
for kmax = 2:2:12
    ...
    a0 = integral(@(t) x_periodic(t), 0, 2) / 2;
    x_reconstructed = x_reconstructed + a0;
    for idx = 1:length(k_values)
        k = k_values(idx);
    
        an_integral = @(t) x_periodic(t).*cos(2*pi*k*t/2);
        bn_integral = @(t) x_periodic(t).*sin(2*pi*k*t/2);
